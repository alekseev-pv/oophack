...
def generate_characters(names):
    ...
    while i < warrior_count:
        name = choice(names)
        characters[i] = Warrior(
            name, randrange(0, 300),
            randrange(0, 300),
            randrange(0, 300)
        )
        i += 1
...
def generate_weapons(names):
    ...
    i = 0
    for item in names:
        weapon_items[i] = Thing(
            item,
            None,
            randrange(0, 100)/1000,
            randrange(0, 1000)
        )
        i += 1
    return weapon_items
...
def fight(player_one, player_two):
    ...
    while player_one_health >= 0 or player_two_health >= 0:
        ...
        print(
            f'{player_two.name} наносит удар по'
            ' {player_one.name} на {hit} урона'
        )
        ...
        print(
            f'{player_one.name} наносит удар по'
            ' {player_two.name} на {hit} урона'
        )
