...
Это мини-игра арена, на которую вы выпустите своих 
персонажей и заставите их сражаться между собой. Играющая сама в себя и выдающая
вам только результат своей работы. 

Отсутствует возможность влиять на игровой процесс.
...
1. Шаг 1 - создаеется произвольное количество вещей с различными параметрами,
    отсортированных по проценту защиты, по возрастанию;

2. Шаг 2 - создается произвольно 10 персонажей, кол-во воинов и паладинов
    произвольно. Имена персонажам тоже рандомные из созданного списка 20 имен.

3. Шаг 3 - персонажи одеваются рандомными вещами. 
    Кому-то 0, кому-то больше, но не более 4 вещей в одни руки;

4. Шаг 4 - персонажи отправляются на арену, и в цикле в произвольном порядке
    выбирается пара Нападающий и Защищающийся.
    1. У Защищающегося вызывается метод вычитания жизни на основе атаки
    `(attack_damage)` Нападающего. 
    2. Количество получаемого урона рассчитывается по формуле: `(attack_damage -`
    `attack_damage * finalProtection)`
    ...
    4. Жизнь вычитается по формуле `(HitPoints - (attack_damage -`
    `attack_damage*finalProtection))`, где `finalProtection` - коэффициент защиты
    в десятичном виде;
...
Для отслеживания процесса битвы выведится информация в таком виде: `{атакующий персонаж} наносит удар по {защищающийся персонаж} на {кол-во урона} урона`
